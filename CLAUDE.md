...
- [x] 다이제스트 보존 정책 — **구현 완료** (2026-08-02, 하이브리드 저장 계층): retro 스윕이 `[sessions] retention_days`(기본 90, 0=끔) 지난 **커밋된** 다이제스트를 git-native ref 트리(`refs/coopera/archive/sessions`, 부모 체인·고정 plumbing identity)로 옮기고 삭제를 스테이징(코드 커밋 동승 — 결정 004 대칭). draft 페이지가 `source:`로 참조 중인 증빙은 보류. ref 갱신 후에만 트리 제거(크래시 안전), push는 비강제 best-effort — 원본은 어차피 main 히스토리에 영구 보존. 회수: `git cat-file -p refs/coopera/archive/sessions:<파일명>`
